...
public class MyPdfPKCS7 {

	// Constructors for creating new signatures

	/**
	 * Assembles all the elements needed to create a signature, except for the
	 * data.
	 * 
	 * @param privKey
	 *            the private key
	 ...
	 * @param hasRSAdata
	 *            <CODE>true</CODE> if the sub-filter is adbe.pkcs7.sha1
	 ...
	 *             on error
	 */
	public MyPdfPKCS7(PrivateKey privKey, Certificate[] certChain, String hashAlgorithm, String provider,
			ExternalDigest interfaceDigest, boolean hasRSAdata)
		...
		if (privKey != null) {
			...
			if (digestEncryptionAlgorithmOid.equals("RSA")) {
				...
				throw new NoSuchAlgorithmException(MessageLocalization.getComposedMessage("unknown.key.algorithm.1",
	...
	/**
	 * Use this constructor if you want to verify a signature using the
	 * sub-filter adbe.x509.rsa_sha1.
	 * 
	 * @param contentsKey
	 *            the /Contents key
	 * @param certsKey
	...
	public MyPdfPKCS7(byte[] contentsKey, byte[] certsKey, String provider) {
		try {
			this.provider = provider;
	 ...
	 */
	@SuppressWarnings({ "unchecked" })
	public MyPdfPKCS7(byte[] contentsKey, PdfName filterSubtype, String provider) {
		...
		try {
			...
			// the certificates
			/*
			 * This should work, but that's not always the case because of a bug
			 * in BouncyCastle:
			 */
			X509CertParser cr = new X509CertParser();
			 ...
			 * 
			 * ASN1Set certSet = null; ASN1Set crlSet = null; while
			 * (content.getObjectAt(next) instanceof ASN1TaggedObject) {
			 * ASN1TaggedObject tagged =
			 * (ASN1TaggedObject)content.getObjectAt(next);
			 * 
			 * switch (tagged.getTagNo()) { case 0: certSet =
			 * ASN1Set.getInstance(tagged, false); break; case 1: crlSet =
			 * ASN1Set.getInstance(tagged, false); break; default: throw new
			 * IllegalArgumentException("unknown tag value " +
			 * tagged.getTagNo()); } ++next; } certs = new
			 * ArrayList<Certificate>(certSet.size());
			 * 
			 * CertificateFactory certFact =
			 * CertificateFactory.getInstance("X.509", new
			 * BouncyCastleProvider()); for (Enumeration en =
			 * certSet.getObjects(); en.hasMoreElements();) { ASN1Primitive obj
			 * = ((ASN1Encodable)en.nextElement()).toASN1Primitive(); if (obj
			 * instanceof ASN1Sequence) { ByteArrayInputStream stream = new
			 * ByteArrayInputStream(obj.getEncoded()); X509Certificate
			 * x509Certificate =
			 * (X509Certificate)certFact.generateCertificate(stream);
			 * stream.close(); certs.add(x509Certificate); } }
			 */
			// the signerInfos
			...
			if (signerInfo.getObjectAt(next) instanceof ASN1TaggedObject) {
				...
				// maybe not necessary, but we use the following line as
				// fallback:
	 ...
	 *            the encryption algorithm. It may must be <CODE>null</CODE> if
	 *            the <CODE>digest</CODE> is also <CODE>null</CODE>. If the
	 *            <CODE>digest</CODE> is not <CODE>null</CODE> then it may be
	 *            "RSA" or "DSA"
	 */
	...
	/**
	 * Class from the Java SDK that provides the functionality of a digital
	 * signature algorithm.
	 */
	private Signature sig;

	/**
	 * The signed digest as calculated by this class (or extracted from an
	 * existing PDF)
	 */
	private byte[] digest;
	 ...
	 * authenticatedAttributes in the signerInfo can also be set. If either of
	 * the parameters is <CODE>null</CODE>, none will be used.
	 * 
	 * @param secondDigest
	 ...
	 * @since 2.1.6
	 */
	public byte[] getEncodedPKCS7(byte secondDigest[], TSAClient tsaClient, byte[] ocsp, Collection<byte[]> crlBytes,
		...
		try {
			...
			// 2006-11-15
			// Sam found Adobe expects time-stamped SHA1-1 of the encrypted
			// digest
			if (tsaClient != null) {
	...
	private DERSet getAuthenticatedAttributeSet(byte secondDigest[], byte[] ocsp, Collection<byte[]> crlBytes,
		...
		try {
			...
			if (sigtype == CryptoStandard.CADES) {
				...
				// If we look into X.690-0207, clause 11.5, we can see that
				// using DER all the components of a sequence having
				// default values shall not be included. According to RFC 5035,
				// 5.4.1.1, definition of ESSCertIDv2, default
	...
	public boolean verify() throws GeneralSecurityException {
		...
		if (isTsp) {
			...
			if (sigAttr != null || sigAttrDer != null) {
				...
				// Stefan Santesson fixed a bug, keeping the code backward
				// compatible
	...
	/**
	 * Helper method that creates the collection of certificates used for the
	 * main signature based on the complete list of certificates and the sign
	 * certificate.
	 */
	 ...
	 * object
	 * 
	 * @return the X.509 certificate revocation lists associated with this
	 *         PKCS#7 object
	 */
